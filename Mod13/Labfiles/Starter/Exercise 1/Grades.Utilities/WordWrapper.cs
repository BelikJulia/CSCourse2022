        ...
        public byte[] EncryptWithX509(byte[] bytesToEncrypt)
        {
            ...
            using (var alg = new AesManaged())
            {
                ...
                using (var outStream = new MemoryStream())
                {
                    ...
                    {
                        var keyForm = new RSAPKCS1KeyExchangeFormatter(provider);
                        var encryptKey = keyForm.CreateKeyExchange(alg.Key, alg.GetType());

                        // Exercise 1: Task 3e: Create byte arrays to get the length of the encryption key and IV. 
                        var keyLength = BitConverter.GetBytes(encryptKey.Length);
                        var ivLength = BitConverter.GetBytes(alg.IV.Length);
                        ...
                        outStream.Write(encryptKey, 0, encryptKey.Length);
                        outStream.Write(alg.IV, 0, alg.IV.Length);
