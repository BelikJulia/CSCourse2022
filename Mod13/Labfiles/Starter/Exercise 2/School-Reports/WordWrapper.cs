        ...
        byte[] DecryptWithX509(byte[] bytesToDecrypt)
        {
            ...
            using (var alg = new AesManaged())
            {
                ...
                using (var inStream = new MemoryStream(bytesToDecrypt))
                {
                    ...
                    var convertKeyLength = BitConverter.ToInt32(keyLength, 0);
                    var convertIvLength = BitConverter.ToInt32(ivLength, 0);

                    // Exercise 2: Task 1g: Determine the starting position and length of the data.
                    var dStartPos = convertKeyLength + convertIvLength + keyLength.Length + ivLength.Length;
                    var dLength = (int)inStream.Length - dStartPos;

                    // Exercise 2: Task 1h: Create the byte arrays for the encrypted key, the IV, and the encrypted data.
                    var encryptionKey = new byte[convertKeyLength];
                    var iv = new byte[convertIvLength];
                    var encryptData = new byte[dLength];

                    // Exercise 2: Task 1i: Read the key, IV, and encrypted data from the in stream.
                    inStream.Read(encryptionKey, 0, convertKeyLength);
                    inStream.Read(iv, 0, convertIvLength);
                    inStream.Read(encryptData, 0, dLength);

                    // Exercise 2: Task 1j: Decrypt the encrypted AesManaged encryption key. 
                    var decryptKey = provider.Decrypt(encryptionKey, false);
                    ...
                    using (var outStream = new MemoryStream())
                    {
                        ...
                        using (var decryptor = alg.CreateDecryptor(decryptKey, iv))
                        {
                            ...
                            using (var decrypt = new CryptoStream(outStream, decryptor, CryptoStreamMode.Write))
                            {
                                ...
                                decrypt.Write(encryptData, 0, dLength);
